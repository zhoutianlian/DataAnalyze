...
def uniform(val_series: pd.Series):
	name = str(val_series.name)
	core, mul = uniform_name(name)
	if mul is not None:
		val_series *= unit[mul]

	return val_series.rename(core)

def uniform_name(name):
	find = re.findall(r'(时间)', name)
	if len(find):
		return '时间', None

	find = re.findall(r'(其中[:：]|加[:：]|减[:：]|[（(]?[一二三四五六七八九十]+[)）、])?([一-龥、-]*)[(（]?(千|万|百万|亿|十亿)?元?[）)]?', name)
	if len(find):
